IDENTIFICATION DIVISION.
        ...
        FILE-CONTROL.
            ...
            SELECT messageFile ASSIGN TO "src/files/message_file.txt"
                ORGANIZATION IS LINE SEQUENTIAL.
            SELECT savedJobFile ASSIGN TO "src/files/saved_jobs.txt"
                ORGANIZATION IS LINE SEQUENTIAL.
        ...
        *> ---------- Login / accounts ----------
            ...
            01  accountCount            PIC 99 VALUE 0.
        ...
        *> ---------- Job Postings ----------
        01 jobPostingData.
            ...
            05 postedByUser          PIC X(30).
        01  jobPostingChoice        PIC X(100).
        ...
        *> --- Applications temp fields ---
            ...
            *> --- Saved Jobs temp fields ---
            01  alreadySaved        PIC X   VALUE "N".
            01  savedJobData.
                05 savedJobID           PIC X(10).
                05 savedByUsername      PIC X(30).

        *> --- Messaging ---
            01 messageRecipientUsername PIC X(30).
            01 isConnectedVar           PIC X VALUE "N".
            ...
            01 messagesFound            PIC X VALUE "N".
        ...
        *> ***************** Subroutines *****************
        ...
        clearFiles.
            ...
            OPEN OUTPUT appliedJobsFile
            CLOSE appliedJobsFile
            OPEN OUTPUT savedJobFile
            CLOSE savedJobFile
        ...
        searchForJobMenu.
            ...
            PERFORM UNTIL exitSearch = "Y"
                ...
                PERFORM displayAndWrite
                MOVE "4. View Saved Jobs" TO messageVar
                PERFORM displayAndWrite
                ...
                ELSE IF menuChoice = "3" OR menuChoice = "View My Applications"
                    PERFORM viewMyApplications
                ELSE IF menuChoice = "4" OR menuChoice = "View Saved Jobs"
                    PERFORM viewSavedJobs
        ...
        findSomeoneMenu.
            MOVE "Please enter their first name or 0 to go back." TO messageVar
            PERFORM displayAndWrite
        ...
        viewProfile.
            *> CLEAR old data to prevent showing stale profile
            MOVE SPACES TO firstName lastName university major aboutMe
            MOVE ZEROS  TO graduationYear
            MOVE ZEROS  TO experienceCount educationCount
        ...
        acceptConnectionRequest.
            ...
            *> REFACTORED: Call the new, shared procedure
            PERFORM removePendingRequest
            EXIT.

        rejectConnectionRequest.
            *> REFACTORED: The logic to remove a request was duplicated.
            *> Now, we just call the new, shared procedure.
            PERFORM removePendingRequest
            EXIT.
        ...
        postJobInternship.
            MOVE "=== POST A JOB/INTERNSHIP ===" TO messageVar
            PERFORM displayAndWrite

            *> Initialize job posting data
            MOVE SPACES TO jobTitle jobDescription jobEmployer jobLocation jobSalary
            MOVE inputUsername TO postedByUser

            *> Get next job ID
            PERFORM getNextJobID
            ...
            *> Collect required fields
            ...
            PERFORM UNTIL jobTitle NOT = SPACES
                READ userInputFile INTO userInputRecord
                    AT END MOVE "Y" TO jobPostingExit EXIT PARAGRAPH
            ...
            MOVE "Enter Job Description (Required):" TO messageVar
            PERFORM displayAndWrite
            PERFORM UNTIL jobDescription NOT = SPACES
                READ userInputFile INTO userInputRecord
                    AT END MOVE "Y" TO jobPostingExit EXIT PARAGRAPH
                    NOT AT END MOVE userInputRecord TO jobDescription
                END-READ
                IF jobDescription = SPACES
                    MOVE "Job Description cannot be blank. Please enter a value." TO messageVar
                    PERFORM displayAndWrite
                END-IF
            END-PERFORM

            MOVE "Enter Employer/Company (Required):" TO messageVar
            PERFORM displayAndWrite
            PERFORM UNTIL jobEmployer NOT = SPACES
                READ userInputFile INTO userInputRecord
                    AT END MOVE "Y" TO jobPostingExit EXIT PARAGRAPH
                    NOT AT END MOVE userInputRecord TO jobEmployer
                END-READ
                IF jobEmployer = SPACES
                    MOVE "Employer/Company cannot be blank. Please enter a value." TO messageVar
                    PERFORM displayAndWrite
                END-IF
            END-PERFORM
            ...
            MOVE "Enter Salary (Optional, press Enter to skip):" TO messageVar
            PERFORM displayAndWrite
            READ userInputFile INTO userInputRecord
                AT END MOVE "Y" TO jobPostingExit EXIT PARAGRAPH
                NOT AT END MOVE userInputRecord TO jobSalary
            END-READ

            *> Save job posting to file
            IF jobPostingExit = "N"
                PERFORM saveJobPosting
                MOVE "Job posting created successfully!" TO messageVar
                PERFORM displayAndWrite
            END-IF
            EXIT.
        ...
        getNextJobID.
            ...
            PERFORM UNTIL endOfFile = "Y"
                READ jobPostingFile INTO jobPostingRecord
                    AT END MOVE "Y" TO endOfFile
                    NOT AT END ADD 1 TO nextJobID
                END-READ
            END-PERFORM
            CLOSE jobPostingFile

            *> Convert numeric ID to string with leading zeros
            MOVE nextJobID TO tempJobID
            MOVE tempJobID TO jobID
            EXIT.
        ...
        viewJobDetails.
                    ...
                    NOT AT END
                        ...
                        IF FUNCTION TRIM(jobID) = FUNCTION TRIM(menuChoice)
                            ...
                            STRING "Posted By: "          DELIMITED BY SIZE
                                    FUNCTION TRIM(postedByUser) DELIMITED BY SIZE
                                INTO messageVar
                            END-STRING
                            PERFORM displayAndWrite
            ...
            MOVE "1. Apply  2. Save  0. Back" TO messageVar
            PERFORM displayAndWrite
            ...
            IF menuChoice = "1" OR menuChoice = "Apply"
                PERFORM registerApplication
            ELSE IF menuChoice = "2" OR menuChoice = "Save"
                PERFORM saveJobForLater
            END-IF
            EXIT.
                    ...
                    NOT AT END
                        UNSTRING applicationRecord DELIMITED BY "|"
                            INTO tempJobID
                                ...
                                tempEmployer
                                tempUsername
                        IF FUNCTION TRIM(tempUsername) = FUNCTION TRIM(inputUsername)
                            ADD 1 TO i
                            MOVE SPACES TO messageVar
                            STRING "Job Title: " DELIMITED BY SIZE
                                    FUNCTION TRIM(tempTitle) DELIMITED BY SIZE
                                INTO messageVar
                            END-STRING
                            PERFORM displayAndWrite
                            ...
                            MOVE SPACES TO messageVar
                            STRING "Employer: " DELIMITED BY SIZE
                                    FUNCTION TRIM(tempEmployer) DELIMITED BY SIZE
                                INTO messageVar
                            END-STRING
                            PERFORM displayAndWrite
        ...
        saveJobForLater.
            MOVE "N" TO alreadySaved
            *> Check for duplicates
            OPEN INPUT savedJobFile
            MOVE "N" TO endOfFile
            PERFORM UNTIL endOfFile = "Y"
                READ savedJobFile INTO savedJobRecord
                    AT END MOVE "Y" TO endOfFile
                    NOT AT END
                        UNSTRING savedJobRecord DELIMITED BY "|"
                            INTO savedJobID
                                savedByUsername
                        IF FUNCTION TRIM(savedJobID) = FUNCTION TRIM(jobID) AND
                            FUNCTION TRIM(savedByUsername) = FUNCTION TRIM(inputUsername)
                            MOVE "Y" TO alreadySaved
                            MOVE "Y" TO endOfFile
                        END-IF
                END-READ
            END-PERFORM
            ...
            IF alreadySaved = "Y"
                MOVE "You have already saved this job." TO messageVar
                PERFORM displayAndWrite
                EXIT PARAGRAPH
            END-IF

            *> Save the job
            OPEN EXTEND savedJobFile
            MOVE SPACES TO savedJobRecord
            STRING FUNCTION TRIM(jobID)        DELIMITED BY SIZE
                    "|"                        DELIMITED BY SIZE
                    FUNCTION TRIM(inputUsername) DELIMITED BY SIZE
                INTO savedJobRecord
            END-STRING
            WRITE savedJobRecord
            CLOSE savedJobFile

            MOVE "Job saved." TO messageVar
            PERFORM displayAndWrite
            EXIT.

        viewSavedJobs.
            MOVE "--- Your Saved Jobs ---" TO messageVar
            PERFORM displayAndWrite
            MOVE "-------------------------" TO messageVar
            PERFORM displayAndWrite
                ...
                READ savedJobFile INTO savedJobRecord
                    ...
                    NOT AT END
                        UNSTRING savedJobRecord DELIMITED BY "|"
                            INTO savedJobID
                                savedByUsername
                        IF FUNCTION TRIM(savedByUsername) = FUNCTION TRIM(inputUsername)
                            PERFORM displaySavedJobDetails
                        END-IF
                END-READ
            END-PERFORM
            CLOSE savedJobFile
            ...
            IF i = 0
                MOVE "You have no saved jobs." TO messageVar
                PERFORM displayAndWrite
            END-IF
        ...
        displaySavedJobDetails.
            MOVE "N" TO eofProfiles *> Re-using flag, 'eofJobPostings'
            OPEN INPUT jobPostingFile
            PERFORM UNTIL eofProfiles = "Y"
                READ jobPostingFile INTO jobPostingRecord
                    AT END MOVE "Y" TO eofProfiles
                    NOT AT END
                        UNSTRING jobPostingRecord DELIMITED BY "|"
                            INTO jobID, jobTitle, jobDescription,
                                jobEmployer, jobLocation, jobSalary,
                                postedByUser
                        IF FUNCTION TRIM(jobID) = FUNCTION TRIM(savedJobID)
                            ADD 1 TO i
                            MOVE SPACES TO messageVar
                            STRING "Job ID: "  DELIMITED BY SIZE
                                    FUNCTION TRIM(jobID) DELIMITED BY SIZE
        ...
        messagesMenu.
            ...
            EVALUATE FUNCTION TRIM(menuChoice)
                WHEN "0" WHEN "Go back"
                    EXIT PARAGRAPH
                WHEN "1" WHEN "Send a New Message"
                    PERFORM sendMessageMenu
                WHEN "2" WHEN "View My Messages"
                    PERFORM viewMessagesMenu
                WHEN OTHER
                    MOVE "Invalid choice, please try again." TO messageVar
                    PERFORM displayAndWrite
            END-EVALUATE
        ...
        sendMessageMenu.
            MOVE "Recipient Username (or 0 to cancel):" to messageVar
            PERFORM displayAndWrite
            ...
            IF FUNCTION TRIM(messageRecipientUsername) = "0"
                EXIT PARAGRAPH
            END-IF
            ...
            ELSE
                ...
                *> FIX: Use logged-in user as sender
                MOVE inputUsername TO sendingUsername
                *> FIX: Use the recipient from user input
                MOVE messageRecipientUsername TO receivingUsername
        ...
        viewMessagesMenu.
            MOVE "--- Your Messages ---" TO messageVar
            PERFORM displayAndWrite
            MOVE "---------------------" TO messageVar
            PERFORM displayAndWrite

            MOVE "N" TO messagesFound
            MOVE "N" TO eofMessages
            OPEN INPUT messageFile

            PERFORM UNTIL eofMessages = "Y"
                READ messageFile INTO messageFileRecord
                    AT END MOVE "Y" TO eofMessages
                    NOT AT END
                        UNSTRING messageFileRecord DELIMITED BY "|"
                            INTO sendingUsername
                                receivingUsername
                                messageContent
                        END-UNSTRING

                        IF FUNCTION TRIM(receivingUsername) = FUNCTION TRIM(inputUsername)
                            MOVE "Y" TO messagesFound
                            MOVE SPACES TO messageVar
                            STRING "From: " DELIMITED BY SIZE
                                    FUNCTION TRIM(sendingUsername)
                                    DELIMITED BY SIZE
                                INTO messageVar
                            END-STRING
                            PERFORM displayAndWrite
            ...
            IF messagesFound = "N"
                MOVE "You have no messages at this time." TO messageVar
                PERFORM displayAndWrite
                MOVE "---------------------" TO messageVar
                PERFORM displayAndWrite
            END-IF
        ...
        isConnected.
            ...
            PERFORM UNTIL endOfFile = "Y"
                READ establishedConnectionFile INTO establishedConnectionRecord
                    ...
                    NOT AT END
                        MOVE establishedConnectionRecord TO establishedConnectionData
                        IF (FUNCTION TRIM(connectedUser1) = FUNCTION TRIM(inputUsername) AND
                            FUNCTION TRIM(connectedUser2) = FUNCTION TRIM(messageRecipientUsername))
                            MOVE "Y" TO isConnectedVar
                            MOVE "Y" TO endOfFile
                        END-IF
                        IF (FUNCTION TRIM(connectedUser2) = FUNCTION TRIM(inputUsername) AND
                            FUNCTION TRIM(connectedUser1) = FUNCTION TRIM(messageRecipientUsername))
                            MOVE "Y" TO isConnectedVar
                            MOVE "Y" TO endOfFile
                        END-IF
                END-READ
        ...
        *> REFACTORED: This new procedure holds the logic that was
        *> duplicated in both acceptConnectionRequest and
        *> rejectConnectionRequest.
        removePendingRequest.
            *> This procedure removes the pending request (stored in
            *> user1Username and user2Username) from connectionFile
            *> by writing all OTHER requests to a temp file and
