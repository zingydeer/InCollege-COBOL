>>SOURCE FORMAT FREE
       ...
       SELECT userOutputFile ASSIGN TO "output.txt"
                ORGANIZATION IS LINE SEQUENTIAL.
       ...
       FILE SECTION.
           ...
           FD connectionFile.
           01 connectionFileRecord PIC X(60).
       ...
       *>Find someone variables
            ...
            01 calleeUsername  PIC X(30).

       *> NEW VARIABLES FOR CONNECTION LOGIC
           01 loggedInUsername      PIC X(30).
           01 connectionRecord.
               02 senderUsername   PIC X(30).
               02 recipientUsername PIC X(30).
           01 pendingRequestCount   PIC 9 VALUE 0.
           01 requestPendingFlag    PIC X VALUE "N".
           01 tempConnectionRecord  PIC X(60).

       PROCEDURE DIVISION.
           OPEN INPUT userInputFile
           OPEN OUTPUT userOutputFile

           Move "Welcome to inCollege by Team Wyoming!"
           TO messageVar
       ...
       validatePassword.
           ...
           IF passwordLength >= 8 AND passwordLength <= 12
               ...
               *> Character validation logic
               ...
               ELSE
                   MOVE "Password must contain uppercase, digit, and special character."
                   TO messageVar
                   PERFORM displayAndWrite
               END-IF
           ELSE
               MOVE "Invalid password length. Must be 8-12 characters."
               TO messageVar
               PERFORM displayAndWrite
           END-IF.
           EXIT.
       ...
       *> New user registration process
       newUserRegistration.
           ...
           IF passwordValid = "Y"
               ...
               MOVE inputUsername TO loggedInUsername
           ELSE
       ...
       *> Post-login main menu
       postLoginMenu.
           ...
           STRING "Welcome, " DELIMITED BY SIZE
                  loggedInUsername DELIMITED BY SPACES
                  "!" DELIMITED BY SIZE
               INTO messageVar
           END-STRING
           PERFORM displayAndWrite
           ...
           PERFORM UNTIL exitMenu = "Y"
               ...
               PERFORM displayAndWrite
               MOVE "4. View My Pending Connection Requests" TO messageVar
               PERFORM displayAndWrite
               MOVE "5. Create/Edit My Profile" TO messageVar
               PERFORM displayAndWrite
               MOVE "6. View My Profile" TO messageVar
               PERFORM displayAndWrite
               MOVE "7. Log out" TO messageVar
               PERFORM displayAndWrite
               ...
               ELSE IF menuChoice = "6"
                   MOVE loggedInUsername to inputUsername
                   PERFORM viewProfile
           ...
           PERFORM UNTIL exitSkills = "Y"
               ...
               ELSE
                   IF menuChoice = "Skill 1" OR menuChoice = "1"
                       ...
                       PERFORM displayAndWrite
                   ELSE IF menuChoice = "Skill 2" OR menuChoice = "2"
                       MOVE "This skill is under construction." TO messageVar
                       PERFORM displayAndWrite
                   ELSE IF menuChoice = "Skill 3" OR menuChoice = "3"
                       MOVE "This skill is under construction." TO messageVar
                       PERFORM displayAndWrite
                   ELSE IF menuChoice = "Skill 4" OR menuChoice = "4"
                       MOVE "This skill is under construction." TO messageVar
                       PERFORM displayAndWrite
                   ELSE IF menuChoice = "Skill 5" OR menuChoice = "5"
                       MOVE "This skill is under construction." TO messageVar
                       PERFORM displayAndWrite
                   END-IF
               END-IF
       ...
       *> Search for job functionality (to be implemented)
       searchForJobMenu.
           MOVE "Search for a job functionality is under construction."
               TO messageVar
           PERFORM displayAndWrite
           EXIT.

       findSomeoneMenu.
           MOVE "Please enter their first and then last name or 6 to go back."
           TO messageVar
           PERFORM displayAndWrite
       ...
       createEditProfile.
           ...
           PERFORM UNTIL profileExit = "Y"
               ...
               ELSE IF profileChoice = "9" OR profileChoice = "Save Profile"
                   PERFORM saveProfile
               ELSE IF profileChoice = "10" OR profileChoice = "Go Back"
                   MOVE "Y" TO profileExit
               ELSE
                   MOVE "Invalid choice, please try again." TO messageVar
                   PERFORM displayAndWrite
       ...
       enterPersonalInfo.
           ...
           PERFORM UNTIL firstName NOT = SPACES
               ...
               END-READ
               IF firstName = SPACES
                   MOVE "First Name cannot be blank. Please enter a value."
                   TO messageVar
                   PERFORM displayAndWrite
               END-IF
           END-PERFORM
           ...
           PERFORM UNTIL lastName NOT = SPACES
               ...
               END-READ
               IF lastName = SPACES
                   MOVE "Last Name cannot be blank. Please enter a value."
                   TO messageVar
                   PERFORM displayAndWrite
               END-IF
           END-PERFORM
           ...
           PERFORM UNTIL university NOT = SPACES
               ...
               IF university = SPACES
                   MOVE "University/College cannot be blank. Please enter a value."
                   TO messageVar
                   PERFORM displayAndWrite
               END-IF
           END-PERFORM
           ...
           PERFORM UNTIL major NOT = SPACES
               ...
               END-READ
               IF major = SPACES
                   MOVE "Major cannot be blank. Please enter a value."
                   TO messageVar
                   PERFORM displayAndWrite
               END-IF
           END-PERFORM
           ...
           PERFORM UNTIL yearValid = "Y"
               READ userInputFile INTO userInputRecord
                   ...
                   NOT AT END
                       ...
                       ELSE
                           MOVE "Invalid year. Please enter a valid 4-digit year."
                           TO messageVar
                           PERFORM displayAndWrite
                       END-IF
               END-READ
       ...
       addExperienceEntry.
           ...
           END-READ
           MOVE "Experience entry added successfully!"
           TO messageVar
           PERFORM displayAndWrite
           EXIT.
       ...
       addEducationEntry.
           ...
           END-READ
           MOVE "Education entry added successfully!"
           TO messageVar
           PERFORM displayAndWrite
           EXIT.
       ...
       saveProfile.
           ...
           PERFORM VARYING j FROM j BY 1 UNTIL j > 3
               ...
               MOVE SPACES TO eduYears(j)
           END-PERFORM
           MOVE expTitle(1)   TO tempProfileRecord(378:49)
           ...
           MOVE eduYears(2)      TO tempProfileRecord(1257:20)
           MOVE eduDegree(3)     TO tempProfileRecord(1277:50)
           MOVE eduUniversity(3) TO tempProfileRecord(1327:50)
           MOVE eduYears(3)      TO tempProfileRecord(1377:20)
           MOVE tempProfileRecord TO profileRecord
           WRITE profileRecord
           CLOSE profileFile
           EXIT.
               ...
               READ profileFile INTO profileRecord
                   ...
                   NOT AT END
                       IF profileRecord(1:30) = inputUsername
                           ...
                           MOVE profileRecord(378:49)   TO expTitle(1)
           ...
           IF entryIndex >= 1 AND entryIndex <= experienceCount
               ...
               READ userInputFile INTO userInputRecord
                   ...
                   NOT AT END
                       ...
                       END-IF
               END-READ
               MOVE "Experience entry updated successfully!"
               TO messageVar
               PERFORM displayAndWrite
           ELSE
           ...
           IF entryIndex >= 1 AND entryIndex <= educationCount
               ...
               READ userInputFile INTO userInputRecord
                   ...
                   NOT AT END
                       ...
                       END-IF
               END-READ
               MOVE "Education entry updated successfully!"
               TO messageVar
               PERFORM displayAndWrite
           ELSE
               MOVE "Invalid entry number." TO messageVar
       ...
       findProfile.
           ...
           PERFORM UNTIL endOfFile = "Y"
               READ profileFile INTO profileRecord
                   AT END
                       MOVE "Y" TO endOfFile
                   NOT AT END
                       IF (profileRecord(31:30) = queryFirstName) AND
                          (profileRecord(61:30) = queryLastName)
                           MOVE profileRecord(1:30) TO calleeUsername
           ...
           *> Load and view the found profile
           MOVE calleeUsername TO inputUsername
           PERFORM loadProfile
           PERFORM viewProfile
           ...
           *> Restore logged-in user's context
           MOVE loggedInUsername TO inputUsername
           EXIT.

       *> ===============================================
       *> NEW PARAGRAPHS FOR WEEK 4
       *> ===============================================

       viewPendingRequests.
           MOVE "--- Pending Connection Requests ---" TO messageVar
           PERFORM displayAndWrite
           MOVE 0 TO pendingRequestCount

           OPEN INPUT connectionFile
           MOVE "N" TO endOfFile
           PERFORM UNTIL endOfFile = "Y"
               READ connectionFile INTO connectionFileRecord
                   AT END
                       MOVE "Y" TO endOfFile
                   NOT AT END
                       MOVE connectionFileRecord TO connectionRecord
                       IF recipientUsername = loggedInUsername
                           MOVE senderUsername TO messageVar
                           PERFORM displayAndWrite
                           ADD 1 TO pendingRequestCount
                       END-IF
               END-READ
           END-PERFORM
       ...
       sendConnectionRequest.
           IF loggedInUsername = calleeUsername
               MOVE "You cannot connect with yourself." TO messageVar
               PERFORM displayAndWrite
               EXIT PARAGRAPH
           END-IF

           PERFORM checkConnectionStatus
           IF requestPendingFlag = "Y"
               EXIT PARAGRAPH
           END-IF

           MOVE loggedInUsername TO senderUsername
           MOVE calleeUsername TO recipientUsername

           OPEN EXTEND connectionFile
               MOVE connectionRecord TO connectionFileRecord
               WRITE connectionFileRecord
           CLOSE connectionFile

           MOVE SPACES TO messageVar
           STRING "Connection request sent to " DELIMITED BY SIZE
                  calleeUsername DELIMITED BY SPACES
                  "." DELIMITED BY SIZE
               INTO messageVar
           END-STRING
           PERFORM displayAndWrite
       ...
       checkConnectionStatus.
           MOVE "N" TO requestPendingFlag
           OPEN INPUT connectionFile
           MOVE "N" TO endOfFile
           PERFORM UNTIL endOfFile = "Y" OR requestPendingFlag = "Y"
               READ connectionFile INTO tempConnectionRecord
                   AT END
                       MOVE "Y" TO endOfFile
                   NOT AT END
                       MOVE tempConnectionRecord(1:30) TO senderUsername
                       MOVE tempConnectionRecord(31:30) TO recipientUsername

                       IF (senderUsername = loggedInUsername AND
                           recipientUsername = calleeUsername)
                           MOVE "You have already sent this user a request."
                               TO messageVar
                           PERFORM displayAndWrite
                           MOVE "Y" TO requestPendingFlag
                       END-IF

                       IF (senderUsername = calleeUsername AND
                           recipientUsername = loggedInUsername)
